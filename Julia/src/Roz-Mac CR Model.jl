...
##Interaction Strength (Flux - Nillson & McCann 2015)
let
...
let 
    ...
    output_dir = "/Users/reillyoconnor/Desktop/R Projects/Fast-Slow-Stability/Julia/Simulation CSVs/CR"
    if !isdir(output_dir)
        mkdir(output_dir)
    end
...
let 
    ...
    for i = 1:length(amaxs)
        ...
        u0 = [1.0, 0.5]
        prob_stoch = SDEProblem(CR_mod!, stoch_CR_Mod!, u0, t_span, p)
        sol_stoch = solve(prob_stoch, ImplicitEM(), reltol = 1e-8, abstol = 1e-8, maxiters = 1e8)
        grid_sol = sol_stoch(ts)
        grid_sol.t
        grid_sol.u
        maxhold[i,1] = amaxs[i]
        maxhold[i,2] = maximum(grid_sol[2,4000:5000])
        stdhold[i] = std(grid_sol[2,4000:5000])
        meanhold[i] = mean(grid_sol[2,4000:5000])
    ...
    xlabel("Attack Rate (a)",fontsize=18,fontweight=:bold)
    ylabel("CV (C)",fontsize=18,fontweight=:bold)
    #ylim(0, 0.75)
    xlim(0.5, 1.15)
    ...
    savefig("/Users/reillyoconnor/Desktop/R Projects/Fast-Slow-Stability/Julia/Figures/Fig 2B - CR CV (attack rate).jpeg", dpi = 300)
...
let 
    ...
    for i = 1:length(amaxs)
        ...
        for j = 1:num_simulations
            ...
            # Calculate statistics for the consumer population (index 2)
            std_vals[j] = std(grid_sol[2, 4000:5000])
            mean_vals[j] = mean(grid_sol[2, 4000:5000])
            cv_vals[j] = std_vals[j] / mean_vals[j]
            taylor_vals[j] = (std_vals[j]^2) / mean_vals[j]
        end

        # Store the average values for each amax
        maxhold[i, 1] = amaxs[i]
        avg_stdhold[i] = mean(std_vals)
        avg_meanhold[i] = mean(mean_vals)
        avg_cvhold[i] = mean(cv_vals)
    ...
    plot(maxhold[1:length(amaxs), 1], avg_cvhold[1:length(amaxs)], color="black", linewidth=3)
    xlabel("Attack Rate (a)", fontsize=18, fontweight=:bold)
    ylabel("Average CV (C)", fontsize=18, fontweight=:bold)
    xlim(0.5, 1.15)
...
let 
    amaxs = 0.58:0.01:1.1
    num_simulations = 5  # Number of simulations for averaging
    ...
    for i = 1:length(amaxs)
        ...
        @threads for j = 1:num_simulations
            # Set parameters for the current value of amax
            p = CRPar(a = amaxs[i], noise = 0.01)
            u0 = [1.0, 0.5]
