...
#load data
df_lpi <- read.csv("../R/Data/Living Planet Index/TS_lpi_detrended.csv")
...
write.csv(df_lpi_ids_final, "../R/Data/TS_lpi_detrended resolved taxonomy.csv")

##### Hatton Dataframes #####
df_metab <- read.csv("../R/Data/Hatton et al 2019/slow_fast_metabolism.csv", header = T)
df_pop_metab <- read.csv("../R/Data/Hatton et al 2019/pop_metab.csv", header = T)
df_growth <- read.csv("../R/Data/Hatton et al 2019/slow_fast_growth.csv", header = T)
...
write.csv(df_metab_gbif_final, "../R/Data/slow_fast_metabolism resolved taxonomy.csv")


#metabolism
df_metab_sub <- df_pop_metab %>%
  filter(Major_taxa == "Mammal") %>%
  mutate(Species = case_when(
    Species == "Arctocephalus townsendi" ~ "Arctocephalus philippii",
    Species == "Spermophilus franklinii" ~ "Poliocitellus franklinii",
...
df_metab_gbif_ids <- df_metab_gbif_ids %>% dplyr::select(gbif_id.ids, Species) %>%
  rename(GBIF_ID = gbif_id.ids)

df_metab_gbif_final <- merge(df_metab_sub, df_metab_gbif_ids, by = "Species")

write.csv(df_metab_gbif_final, "../R/Data/slow_fast_pop_metabolism resolved taxonomy.csv")
...
write.csv(df_grw_gbif_final, "../R/Data/slow_fast_growth resolved taxonomy.csv")


#rmax subset - count method rm only
Growth <- read.csv("../R/Data/Hatton et al 2019/Growth.csv", header = T)
...
write.csv(df_grw_gbif_final, "../R/Data/slow_fast_growth resolved taxonomy - count method.csv")



##### Mhyrvold Database #####
df_body_sizes <- read.csv("../R/Data/Amniote Life History Database/Data_Files/Amniote_Database_Aug_2015.csv")
...
write.csv(df_bs_gbif_final, "../R/Data/Amniote database resolved taxonomy.csv")

##### Beccari et al 2024 Data #####
#Beccari et al 2024
df_fast_slow <- read.table("../R/Data/Beccari et al 2024/Mammal_BMcor_Def copy.txt", header = T, sep = "", dec = ".")
...
write.csv(df_fs_gbif_final, "../R/Data/Beccari Fast-Slow resolved taxonomy.csv")
